...
let cameraCount = BinaryTreeCamera().minCameraCover(treeCamera)
print(cameraCount)

print("\n -- Binary Tree Cousins --")
let cousinTree = Tree(value: 1)
cousinTree.left = Tree(value: 2)
cousinTree.right = Tree(value: 3)
cousinTree.left?.right = Tree(value: 4)
cousinTree.right?.right = Tree(value: 5)
let cousins = CousinsBinaryTree().isCousins(root: cousinTree, x: 5, y:4)
print(cousins)

print("\n --- Boundary of Binary tree ---")
let boundaryTree = Tree(value: 1)
boundaryTree.right = Tree(value: 2)
boundaryTree.right?.left = Tree(value: 3)
boundaryTree.right?.right = Tree(value: 4)
let boundaryTreeResult = BoundaryBinaryTree().boundaryOfBinaryTree(boundaryTree)
print(boundaryTreeResult)

print(" -- Largest Value Each Row ----")
let largestValueTree = Tree(value: 1)
largestValueTree.left = Tree(value: 3)
largestValueTree.right = Tree(value: 2)
largestValueTree.left?.left = Tree(value: 5)
largestValueTree.left?.right = Tree(value: 3)
largestValueTree.right?.right = Tree(value: 9)
let largestValueResult = LargestValueEachRow().largestValues(largestValueTree)
print(largestValueResult)

print(" -- Smallest Sub tree wyth all the Deepest Nodes")
let smallestSubTree = Tree(value: 3)
smallestSubTree.left = Tree(value: 5)
smallestSubTree.right = Tree(value: 1)
smallestSubTree.left?.left = Tree(value: 6)
smallestSubTree.left?.right = Tree(value: 2)
smallestSubTree.left?.right?.left = Tree(value: 7)
smallestSubTree.left?.right?.right = Tree(value: 4)
let smallestSubTreeResult = SmallesSubTreeWithDeepestNodes().subTreeWithAllDeepest(smallestSubTree)
print("\(smallestSubTreeResult?.value) --> \(smallestSubTreeResult?.left?.value) --> \(smallestSubTreeResult?.right?.value)  ")
